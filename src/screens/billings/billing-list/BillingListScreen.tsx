...
const BillingListScreen = (props: PaymentListComponentProps) => {
    ...
    const clientPendingPaymentColumn: ITableColumn[] = useMemo<any>(() => [
        ...
        },
        {
            ...
            align: 'center',
            render: (item: any) => {
                return <LinkComponent route={CommonService._routeConfig.SingleClientBillingDetails(item?._id)+ '?referrer='+ location.pathname + '&type='+'invoice' }>
                    {
                        (item?.appointment_details.appointment_number).length > 10 ?
                            <ToolTipComponent
                                tooltip={item?.appointment_details.appointment_number}
    ...
    const clientCompletePaymentListColumn: ITableColumn[] = useMemo<any>(() => [
        {
            ...
            render: (item: any) => {
                return <LinkComponent route={CommonService._routeConfig.SingleClientBillingDetails(item?._id)+ '?referrer='+ location.pathname + '&type='+'receipt' }>
                    {item?.receipt_number}
                </LinkComponent>
            }
        },
