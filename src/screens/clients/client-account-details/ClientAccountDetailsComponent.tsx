...
const ClientAccountDetailsComponent = (props: ClientAccountDetailsComponentProps) => {
    ...
    console.log(clientAccountDetails);
            ...
            <CardComponent title={"Communication and Referral Details"}>
                                   ...
                                   description={clientAccountDetails?.referral_details.source?.title || "-"}/>
                {
                    clientAccountDetails?.referral_details.source?.code === "friends_family_colleague" && <>
                        <DataLabelValueComponent label={"Name"}> {clientAccountDetails?.referral_details?.source_info_name} </DataLabelValueComponent>
                        <DataLabelValueComponent label={"Phone Number"}> {clientAccountDetails?.referral_details?.source_info_phone} </DataLabelValueComponent>
                        <DataLabelValueComponent label={"Email"}> {clientAccountDetails?.referral_details?.source_info_email} </DataLabelValueComponent>
                        <DataLabelValueComponent label={"Relationship"}> {clientAccountDetails?.referral_details.source_info_relationship?.title} </DataLabelValueComponent>
                    </>
                }
                {
                    clientAccountDetails?.referral_details.source?.code === "social_media" && <>
                        <DataLabelValueComponent label={"Social Media"}> {clientAccountDetails?.referral_details?.source_info_name?.title} </DataLabelValueComponent>
                    </>
                }
                {
                    clientAccountDetails?.referral_details.source?.code === "other" && <>
                        <DataLabelValueComponent label={"Other"}> {clientAccountDetails?.referral_details?.source_info_name} </DataLabelValueComponent>
                    </>
                }
            </CardComponent>
        </div>
